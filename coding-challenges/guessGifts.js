...
var presents = [ { size: 'small', clatters: 'yes', weight: 'light' },
  { size: 'medium', clatters: 'a bit', weight: 'medium' } ]
  ...
  function guessGifts(wishlist, presents) {
      return wishlist.filter( w => {
        return presents.some(p => {
          return p.size === w.size  && p.clatters === w.clatters && p.weight === w.weight
          })
      }).map(x => x.name)
    }
