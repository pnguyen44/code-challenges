// 6 kyu - The maximum sum value of ranges -- Simple version

// Given an array arr that contains some integers(positive, negative or 0), and a range
// list such as [[start1,end1],[start2,end2],...], start and end are the index of arr and
// start always less than end. Your task is to calculate the sum value of each
// range (start index and end index are both inclusive), and return the maximum sum value.
//
// For example:
//
...
//  maxSum([1,-2,3,4,-5,-4,3,2,1],[[1,4],[2,5]]) === 0
  ...
  function maxSum(arr,range){
    ...
    for (let i = 0; i < range.length; i++) {
      for (let j = range[i][0]; j < range[i][1] + 1; j++) {
        sum += arr[j]
      }
      sumArr.push(sum)
      sum = 0
